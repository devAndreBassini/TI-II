...
public class XDAO {
	...
	public boolean inserirUsuario(XUsuario usuario) {
		boolean status = false;
		try {
	...
	public boolean atualizarUsuario(XUsuario usuario) {
		boolean status = false;
		try {
			Statement st = conexao.createStatement();
	...
	public XUsuario[] getUsuarios() {
		XUsuario[] usuarios = null;
		...
		try {
			...
			if (rs.next()) {
				rs.last();
				usuarios = new XUsuario[rs.getRow()];
				rs.beforeFirst();

				for (int i = 0; rs.next(); i++) {
					usuarios[i] = new XUsuario(rs.getInt("codigo"), rs.getString("login"), rs.getString("senha"),
	...
	public XUsuario[] getUsuariosMasculinos() {
		XUsuario[] usuarios = null;
